...
public class Damage : MonoBehaviour {
	...
	{	
		if (damageOnCollision) {
            ...
            if (this.tag == collision.gameObject.tag) // if the player got hit with it's own bullets, ignore it
                return;
	...
	{
		if (continuousDamage) {
			if ((collision.gameObject.tag == "Target" || collision.gameObject.tag == "Player") && collision.gameObject.GetComponent<Health> () != null) {	// is only triggered if whatever it hits is the player
